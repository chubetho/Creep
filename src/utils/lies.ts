...
import { IS_BLINK, IS_GECKO, IS_WEBKIT, IS_WORKER_SCOPE } from './helpers'
...
function createLieRecords() {
  ...
  return {
    ...
    documentLie: (name: string, lie: string | string[]) => {
      const isArray = Array.isArray(lie)
      if (records[name]) {
        if (isArray)
...
function queryLies({
  ...
  const lies: Record<string, boolean> = {
    ...
    'failed call interface error': failsTypeError({
      spawnErr: () => {
        new (apiFunction as any)(); apiFunction.call(proto)
      },
    }),
    'failed apply interface error': failsTypeError({
      spawnErr: () => {
        new (apiFunction as any)(); apiFunction.apply(proto)
      },
    }),
    'failed new instance error': failsTypeError({
      spawnErr: () => new (apiFunction as any)(),
    }),
...
function getPluginLies(plugins: PluginArray, mimeTypes: MimeTypeArray) {
  const lies = [] // collect lie types
  const pluginsOwnPropertyNames = Object.getOwnPropertyNames(plugins).filter(name => Number.isNaN(+name))
  const mimeTypesOwnPropertyNames = Object.getOwnPropertyNames(mimeTypes).filter(name => Number.isNaN(+name))
