...
const pluralify = (len: number) => len > 1 ? 's' : ''
const count = (arr: Array<unknown>) => arr && arr.constructor.name === 'Array' ? `${arr.length}` : '0'

function getDiffs({ stringA, stringB, charDiff = false, decorate = diff => `[${diff}]` }: { stringA: string, stringB: string, charDiff?: boolean, decorate?: (diff: string) => string },
...
// modal component
function modal(name: string, result: string[], linkname = 'details') {
  if (!result.length)
    return ''
