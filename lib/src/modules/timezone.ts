...
export function getTimezone() {
  ...
  const getTimezoneOffsetHistory = ({ year, city = undefined }: { year: string, city?: string }) => {
  ...
  const binarySearch = (list: string[], fn: (arr: string[]) => string[]): string[] => {
  ...
  const decryptLocation = ({ year, timeZone }: { year: string, timeZone: string }) => {
    const system = getTimezoneOffsetHistory({ year })
    const resolvedOptions = getTimezoneOffsetHistory({ year, city: timeZone })

    // get city region set
    const decryption = (
      system === resolvedOptions
        ? [timeZone]
        : binarySearch(cities, cities => cities.filter(city => system === getTimezoneOffsetHistory({ year, city })),
        ))
