import { PlatformClassifier, type PlatformClassifierValue } from './types'
...
const ENGINE_IDENTIFIER = getEngine() ?? -1
...
function braveBrowser() {
  const brave = (
    ...
    && (navigator.brave as any).isBrave.toString() === 'function isBrave() { [native code] }'
  )
  return brave
}
...
function getBraveUnprotectedParameters(parameters: Record<string, string>) {
...
function getReportedPlatform(userAgent: PlatformClassifierValue, platform?: string): PlatformClassifierValue[] {
  // user agent os lie
  const userAgentOS = (
    // order is important
    /win(?:dows|16|32|64|95|98|nt)|wow64/i.test(userAgent)
...
function getUserAgentPlatform({ userAgent, excludeBuild = true }: { userAgent: string, excludeBuild: boolean }) {
  if (!userAgent)
    return 'unknown'
      ...
      return identifiers
        .map((x) => {
          if (appleRelease.test(x)) {
            ...
            const isOSX = version.startsWith('10')
...
function computeWindowsRelease({ platform, platformVersion, fontPlatformVersion }: { platform: PlatformClassifierValue, platformVersion: string, fontPlatformVersion: string }) {
...
function attemptWindows11UserAgent({ userAgent, userAgentData, fontPlatformVersion }: { userAgent: string, userAgentData: { platformVersion: string, platform: PlatformClassifierValue }, fontPlatformVersion: string }) {
  const { platformVersion, platform } = userAgentData || {}
  const windowsRelease = computeWindowsRelease({ platform, platformVersion, fontPlatformVersion })
  return (
...
function isUAPostReduction(userAgent: string) {
...
function getUserAgentRestored({ userAgent, userAgentData, fontPlatformVersion }: { userAgent: string, userAgentData: { brands: string[], uaFullVersion: string, platformVersion: string, model: string, bitness: string }, fontPlatformVersion: string }) {
  ...
  const userAgentRestored = userAgent
    ...
    .replace(/(; Win64; x64| x86_64)/, (_, p1) => bitness === '64' ? p1 : '')

  return userAgentRestored
}

function formatEmojiSet(emojiSet: string[], limit = 3) {
...
export { IS_BLINK, IS_GECKO, IS_WEBKIT, JS_ENGINE, LIKE_BRAVE, LIKE_BRAVE_RESISTANCE, ENGINE_IDENTIFIER, braveBrowser, getBraveMode, getBraveUnprotectedParameters, getOS, getReportedPlatform, USER_AGENT_OS, PLATFORM_OS, decryptUserAgent, getUserAgentPlatform, computeWindowsRelease, attemptWindows11UserAgent, isUAPostReduction, getUserAgentRestored, formatEmojiSet, EMOJIS, CSS_FONT_FAMILY, hashSlice, Analysis, LowerEntropy, getGpuBrand }
